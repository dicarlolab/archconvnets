...
class FeatureWriterDriver(Driver):
    ...
    def on_finish_batch(self):
        paths_out = [os.path.join(fp, 'data_batch_%d' % self.batchnum) for fp in self.feature_paths]
        for path_out, ftrs in zip(paths_out, self.ftr_list):
            pickle(path_out, {'data': ftrs, 'labels': self.data[1]})
