	...
	for x in range(output_sz2):
		...
		for y in range(output_sz2):
			for filter in range(n_filters):
				...
				s1 = output_patch.shape[0]
				s2 = output_patch.shape[1]
				output_patch_flat = output_patch.reshape((s1*s2, n_imgs))
				deriv_patch_flat = deriv_patch.reshape((in_channels, filter_sz, filter_sz, s1*s2, n_imgs))
