...
def f(y):
	...
	o += interpolate_simp(o_content, g3)
...
def g(y):
	...
	do_dw1 = np.einsum(do_dw1 + do_content_dw1, range(4), g3, [0,4], range(4))
	do_dw1 += np.einsum(o_prev + o_content, [0,1], dg3_dw1, [0,4,2,3], range(4))
	...
	do_dw2 = np.einsum(do_dw2 + do_content_dw2, range(4), g3, [0,3], range(4))
	do_dw2 += np.einsum(o_prev + o_content, [0,1], dg3_dw2, [0,2,3], range(4))
	
	# w3:
	dg3_dw3 = sq_dF(w3, g2, g3)
	
	do_dw3 = np.einsum(do_dw3 + do_content_dw3, [0,1,2], g3, [0,3], [0,1,2])
	do_dw3 += np.einsum(o_prev + o_content, [0,1], dg3_dw3, [0,2], [0,1,2])
