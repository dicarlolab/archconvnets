...
shift_out = np.random.normal(size=(C, n_shifts)) * SCALE
...
dmem_dwr = np.zeros((C, M, mem_length, n_in))
...
def g(y):
	...
	dadd_mem_dgw = add_mem_dgw(add_out)
	#dgw_dww = 
	...
	do_in_dw3 = np.einsum(do_dw3 + do_content_dw3, range(4), g3, [0,3], range(4))
	do_in_dw3 += np.einsum(o_prev + o_content, [0,1], dg3_dw3, [0,2], [0,1,2])[:,:,np.newaxis]#np.einsum(o_prev + o_content, [0,1], dg3_dw3, [0,2], [0,1,2])
	
	do_dw3 = np.einsum(do_in_dw3, [0,1,2,5], do_do_in, [3,4,0,1], [3,4,2,5])
	...
	do_in_dw3 = np.einsum(do_dw3 + do_content_dw3, range(4), g3, [0,3], range(4))
	do_in_dw3 += np.einsum(o_prev + o_content, [0,1], dg3_dw3, [0,2], [0,1,2])[:,:,np.newaxis]
