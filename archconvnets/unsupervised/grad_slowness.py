...
def test_grad_slowness(feature_path, batch, tmp_model, neuron_ind, in_channels, filter_sz, n_filters, n_imgs, output_sz, frames_per_movie, deriv_prefix_name, compute_grad=True,thresh=1):
	...
	output_deriv = output_deriv.reshape((in_channels*filter_sz**2, 1, output_sz**2, n_imgs))
        ...
        conv_out_nmean_std_pad = conv_out_nmean_std[np.newaxis]
        conv_out_nmean_pad = conv_out_nmean[np.newaxis]

	if compute_grad == True:
		# we want to do this, but it takes too much memory, so we need to slice through:
		#grad_ld1 = (output_deriv*conv_out_nmean_pad).sum(2) / conv_out_nmean_std_pad
		#print 'starting grad_ld1 computation'
		grad_ld1 = np.zeros((in_channels*filter_sz**2, n_filters, n_imgs),dtype='float32')
		for img in range(n_imgs):
			#print '1 ', img
			grad_ld1[:,:,img] = (output_deriv[:,:,:,img]*conv_out_nmean_pad[:,:,:,img]).sum(2) / conv_out_nmean_std_pad[:,:,img]
		# output_deriv*conv_out_nmean: in_channels*filter_sz**2, n_filters, output_sz**2, n_imgs
        for img in range(0, n_imgs-1):
		#print '2 ', img
                if (img % 2) == 0:#2) == 0: #if (((img-2) % frames_per_movie) != 0) and (((img+2) % frames_per_movie) != 0) and (((img+1) % frames_per_movie) != 0) and (((img) % frames_per_movie) != 0) and (((img-1) % frames_per_movie) != 0): # skip movie boundaries
                        ...
                        if compute_grad == True:
				grad_l = (output_deriv[:,:,:,img]*conv_out_nmean_pad[:,:,:,img+1]).sum(2)
				grad_l += (output_deriv[:,:,:,img+1]*conv_out_nmean_pad[:,:,:,img]).sum(2)
        ...
        loss = -corrs_loss
	grad *= 1 - 2*((corrs_loss/n_corrs) > thresh)
        print corrs_loss/n_corrs, time.time() - t_start, 1 - 2*((corrs_loss/n_corrs) > 0.64)
        return grad
