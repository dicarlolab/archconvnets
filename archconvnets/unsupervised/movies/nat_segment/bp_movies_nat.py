...
N_C = len(np.unique(np.int32(np.load('/home/darren/archconvnets/archconvnets/unsupervised/movies/nat_segment/L3_labels.npy'))[:N_BP_IMGS]))
...
z = np.load('/home/darren/archconvnets/archconvnets/unsupervised/movies/nat_segment/combined_batch')

#z['labels'] = np.int32(np.linspace(0,N_C-1,N_BP_IMGS))
z['labels'] = np.int32(np.load('/home/darren/archconvnets/archconvnets/unsupervised/movies/nat_segment/L3_labels.npy'))
...
while True:
	...
	#FL = np.single(np.random.normal(scale=FL_scale, size=(N_C, n3, max_output_sz3, max_output_sz3)))
	for s in range(N_BP_IMGS/N_IMGS):
