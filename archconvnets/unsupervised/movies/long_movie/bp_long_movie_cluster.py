...
file_name = '/home/darren/F1_' + str(N_C) + BP_STR + '_' + str(EPS_E) + 'eps_' + str(N) + 'N_long_movie_obj_cluster.mat'
...
frames_shuffled = range(8*8*100*150)
random.seed(666)
random.shuffle(frames_shuffled)

z = np.load('/home/darren/long_movie_batch/labels_pred')
obj_order_pred = z['obj_order'][:6398]
...
while True:
	for batch in range(5,97):
		...
		##################
		# load train imgs into buffers (movies)
		z = np.load('/home/darren/long_movie_batch/data_batch_' + str(batch))
		
		# remap temporaly continously ordered label predictions to shuffled batch order
		for frame_remapped in range(10000):
			frame_orig = frames_shuffled[(batch-1)*10000 + frame_remapped]
			if (frame_orig / 150) >= len(obj_order_pred):
				#print 'missing prediction....'
				z['labels_obj'][frame_remapped] = np.random.randint(64)
			else:
				z['labels_obj'][frame_remapped] = obj_order_pred[frame_orig / 150]
		
		z['labels'] = z['labels_obj']
		z['mean'] = np.zeros((3*32*32,1), dtype='single')
