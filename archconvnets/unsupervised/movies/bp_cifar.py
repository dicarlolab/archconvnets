...
while True:
	for s in range(500):
		...
		set_buffer(imgs_pad[s_imgnet*N_IMGS:(s_imgnet+1)*N_IMGS], IMGS_PAD, gpu=GPU_S)
		...
		set_buffer(FL, FL_IND, filter_flag=1, gpu=GPU_S)
		...
		Ys = np.ascontiguousarray(Y_train[:,s_imgnet*N_IMGS:(s_imgnet+1)*N_IMGS])
		...
		pred_buffer(FL_IND, MAX_OUTPUT3, PRED, Ys, gpu=GPU_S) # === np.einsum(FL, range(4), max_output3, [4,1,2,3], [0,4]) - Ys
