...
def test_grad_grad(x):
	global transpose_norm, l2_norm, fourier_norm
	...
	if transpose_norm == np.inf:
		transpose_norm = 0*0.1 * (corrs_loss) / loss_t
	...
	if fourier_norm == np.inf:
		fourier_norm = 100 * corrs_loss / fourier_loss
	
	grad = grad_ldt.ravel() + transpose_norm*grad_t + fourier_norm*grad_f
	
	if np.mean(np.abs(corrs)) < 0.1:
		loss_t = 0
		grad_t = 0
	
	loss = -corrs_loss + transpose_norm*loss_t + fourier_norm * fourier_loss
	
	print loss, time.time() - t_start, t_conv, np.mean(np.abs(corrs)), corrs_loss/n_corrs, fourier_norm*fourier_loss
...
base_batches = np.arange(80000, 80000+2)#[80000,80001,80002,80003]
