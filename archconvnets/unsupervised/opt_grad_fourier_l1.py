...
def test_grad(x):
	...
	grad_f += grad_f2
        ...
        #########
	scale_channel = 5e2#1e3###############1*1e-1#1e0#1e3
	scale_fourier = 1#5############## 1*1e-4#1e-3#1
	...
	if fourier_loss < thresh:
	    ...
	    neuron_indt = 6
            weight_indt = 5
	    s = model4['model_state']['layers'][neuron_indt]['inputLayers'][0]['weights'][0].shape
	    w = np.random.normal(0,.01,s).astype('float32')
	    model4['model_state']['layers'][neuron_indt]['inputLayers'][0]['weights'][0] = w.reshape(s)
	    model4['model_state']['layers'][weight_indt]['weights'][0] = w.reshape(s)
	    ...
	    model4['model_state']['layers'][neuron_indt]['inputLayers'][0]['weights'][0] = w.reshape(s)
            ...
            weight_indt = 11
            s = model4['model_state']['layers'][weight_indt]['weights'][0].shape
            w = np.random.normal(0,.01,s).astype('float32')
            model4['model_state']['layers'][weight_indt]['biases'] = np.ones_like(model4['model_state']['layers'][weight_indt]['biases'], dtype='float32')	
...
model4 = unpickle('/home/darren/imgnet_3layer_256_relu_prelim.model')
#model4 = unpickle('/home/darren/imgnet_3layer_256_relu.model')
#model4 = unpickle('/home/darren/imgnet_3layer_128_relu_prelim.model')
