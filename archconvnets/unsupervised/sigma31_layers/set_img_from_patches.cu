...
// int: N_C
static PyObject *set_img_from_patches(PyObject *self, PyObject *args){
	...
	PyArrayObject *imgs_in, *inds_in, *imgs_new_in;
	...
	if (!PyArg_ParseTuple(args, "O!O!O!O!O!O!iiiO!O!O!", 
		...
		&s1, &s2, &s3, &PyArray_Type, &imgs_in, &PyArray_Type, &inds_in, &PyArray_Type, &patches_in)) 
		return NULL;
	...
	if (NULL == output_switches3_x_in || NULL == output_switches3_y_in ||
		...
		NULL == imgs_in || NULL == inds_in || NULL == patches_in)  return NULL;
	...
	dims[2] = img_sz;
	dims[3] = img_sz;
	imgs_new_in = (PyArrayObject *) PyArray_FromDims(4, dims, NPY_FLOAT);
	imgs_new = (float *) imgs_new_in -> data;	
	
	int f1, channel, a1_x, a1_y, f2, a2_x, a2_y, f3, a3_x, a3_y, z1, z2, img, ind;
	...
	for(channel = 0; channel < 3; channel++){
		for(img = 0; img < N_IMGS; img++){
			for(a1_x_global = 0; a1_x_global < img_sz; a1_x_global++){ for(a1_y_global = 0; a1_y_global < img_sz; a1_y_global++){
		imgs_new[I_IND(img, channel,a1_x_global,a1_y_global)] = imgs[I_IND(img, channel,a1_x_global,a1_y_global)];
	}}}}
	...
	for(ind = 0; ind < n_inds; ind++){
		...
		for(img = 0; img < N_IMGS; img++){
			...
			imgs_new[I_IND(img, channel,a1_x_global,a1_y_global)] = patches[P_IND(img, ind)];
		} // img 
	} // ind
	
	return PyArray_Return(imgs_new_in);
}
