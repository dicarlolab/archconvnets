...
// int: N_C
static PyObject *compute_patch_inds(PyObject *self, PyObject *args){
	PyObject * list;
	...
	PyArrayObject *imgs_in, *labels_in, *inds_in;
	...
	if (!PyArg_ParseTuple(args, "O!O!O!O!O!O!iiiO!O!iO!", 
		...
		&s1, &s2, &s3, &PyArray_Type, &labels_in, &PyArray_Type, &imgs_in, &N_C, &PyArray_Type, &inds_in)) 
		return NULL;
