...
#define O3_IND(A,B,C,D)((D) + (C)*max_output_sz3 + (B)*max_output_sz3*max_output_sz3 + (A)*max_output_sz3*max_output_sz3*n3)
#define O2_IND(A,B,C,D)((D) + (C)*max_output_sz2 + (B)*max_output_sz2*max_output_sz2 + (A)*max_output_sz2*max_output_sz2*n2)
#define O1_IND(A,B,C,D)((D) + (C)*max_output_sz1 + (B)*max_output_sz1*max_output_sz1 + (A)*max_output_sz1*max_output_sz1*n1)

#define S1_IND(A,B,C,D,E)((E) + (D)*s1 + (C)*s1*s1 + (B)*s1*s1*n1 + (A)*s1*s1*n1*3)
#define S2_IND(A,B,C,D,E)((E) + (D)*s2 + (C)*s2*s2 + (B)*s2*s2*n1 + (A)*s2*s2*n1*n2)
#define S3_IND(A,B,C,D,E)((E) + (D)*s3 + (C)*s3*s3 + (B)*s3*s3*n2 + (A)*s3*s3*n2*n3)
#define SL_IND(A,B,C,D)((D) + (C)*max_output_sz3 + (B)*max_output_sz3*max_output_sz3 + (A)*max_output_sz3*max_output_sz3*n3)

#define I_IND(A,B,C,D)((D) + (C)*img_sz + (B)*img_sz*img_sz + (A)*img_sz*img_sz*3)
								...
								for(a1_x = 0; a1_x < s1; a1_x++){
									...
									for(a1_y = 0; a1_y < s1; a1_y++){
										...
										for(channel = 0; channel < 3; channel++){
											sigma31_L1[S1_IND(cat,channel,f1,a1_x,a1_y)] += imgs[I_IND(img,channel,a1_x_global,a1_y_global)];
											img_sum += imgs[I_IND(img,channel,a1_x_global,a1_y_global)];
										}
										sigma31_L2[S2_IND(cat, f2,f1,a2_x,a2_y)] += img_sum;
										sigma31_L3[S3_IND(cat, f3,f2,a3_x,a3_y)] += img_sum;
										sigma31_FL[SL_IND(cat, f3,z1,z2)] += img_sum;
									}
								} // a1_x
