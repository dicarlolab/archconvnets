__global__ void kernel_bp_patch_sigma31_uns(float * deriv, float * sigma_imgs, float * imgs, float * F1, float * F2, float * F3, float * FL, 
	...
	/*int f3_c = threadIdx.y;
	f3_i = &f3_c;
	f3_sz = 1;*/
	
	int a3_y_c = threadIdx.y;
	a3_y_i = &a3_y_c;
	a3_y_sz = 1;
	...
	for(f3=0; f3 < f3_sz; f3++){ for(z1=0; z1 < z1_sz; z1++){ for(z2=0; z2 < z2_sz; z2++){ for(a3_x=0; a3_x < a3_x_sz; a3_x++){ for(a3_y=0; a3_y < a3_y_sz; a3_y++){
		...
		///////////////////////////////////////////////////// uns
		// pool3 -> conv3
		a3_x_global = output_switches3_x[O3_IND(*img_i,*f3_i,*z1_i,*z2_i)] + *a3_x_i;
		a3_y_global = output_switches3_y[O3_IND(*img_i,*f3_i,*z1_i,*z2_i)] + *a3_y_i;
		...
		for(f2=0; f2 < f2_sz; f2++){ for(a2_x=0; a2_x < a2_x_sz; a2_x++){ for(a2_y=0; a2_y < a2_y_sz; a2_y++){
			...
			// pool2 -> conv2
			a2_x_global = output_switches2_x[O2_IND(*img_i,*f2_i,a3_x_global,a3_y_global)] + *a2_x_i;
			a2_y_global = output_switches2_y[O2_IND(*img_i,*f2_i,a3_x_global,a3_y_global)] + *a2_y_i;
			...
			for(f1=0; f1 < f1_sz; f1++){  for(a1_x=0; a1_x < a1_x_sz; a1_x++){  for(a1_y=0; a1_y < a1_y_sz; a1_y++){ 
				...
				a1_x_global = output_switches1_x[O1_IND(*img_i,*f1_i,a2_x_global,a2_y_global)] + *a1_x_i;
				a1_y_global = output_switches1_y[O1_IND(*img_i,*f1_i,a2_x_global,a2_y_global)] + *a1_y_i;
				  ...
				  for(cat=0; cat < cat_sz; cat++){ 
					...
					F_prod_pred = F_prod * pred[*cat_i + N_C**img_i];
					
					for(channel=0; channel < channel_sz; channel++){
						temp_deriv -= F_prod_pred * imgs[I_IND(*img_i, *channel_i,a1_x_global,a1_y_global)];
