...
file_name = '/home/darren/imgnet_92_model_randhalf.mat'
...
FL_imgnet = np.single(np.random.normal(scale=FL_imgnet_scale, size=(2, n3, max_output_sz3, max_output_sz3)))
FL_imgnet_full = np.single(np.random.normal(scale=FL_imgnet_scale, size=(999, n3, max_output_sz3, max_output_sz3)))
...
err_imgnet_test_full_plot = []
class_err_imgnet_test = []
class_err_imgnet_test_full = []

cat_inds = np.arange(999)
...
Y_test_imgnet_reduced = np.zeros((2,N_TRAIN_IMGNET),dtype='single')
labels_test_imgnet = np.zeros(N_TRAIN_IMGNET, dtype='int')
labels_test_imgnet_reduced = np.zeros(N_TRAIN_IMGNET, dtype='int')
...
Y_test_imgnet_reduced[0] = Y_test_imgnet[cat_inds[:500]].sum(0)
Y_test_imgnet_reduced[1] = Y_test_imgnet[cat_inds[500:]].sum(0)

cat_step = 0
for cat in cat_inds:
	labels_test_imgnet_reduced[labels_test_imgnet == cat] = (cat_step >= 500)
	cat_step += 1
...
while True:
	...
	if s_imgnet == 0:
		...
		Y_imgnet_reduced = np.zeros((2,IMGNET_BATCH_SZ),dtype='single')
		...
		Y_imgnet_reduced[0] = Y_train[cat_inds[:500]].sum(0)
		Y_imgnet_reduced[1] = Y_train[cat_inds[500:]].sum(0)
	...
	pred_full = np.einsum(FL_imgnet_full, range(4), max_output3, [4,1,2,3], [0,4])
	
	pred_m_Y = Y_imgnet_reduced[:,s_imgnet*64:(s_imgnet+1)*64] - pred
	pred_m_Y_full = Y_train[:,s_imgnet*64:(s_imgnet+1)*64] - pred_full
	...
	### return
	dFL_imgnet_full += np.einsum(max_output3, range(4), pred_m_Y_full, [4,0], [4,1,2,3])
	...
	FL_imgnet_full += (dFL_imgnet_full + MOM_WEIGHT*dFL_imgnet_full_mom)*EPS / 64
	...
	if step % SAVE_FREQ == 0:
		...
		err_imgnet_test_plot.append(np.mean((pred - Y_test_imgnet_reduced)**2))
		err_imgnet_test_full_plot.append(np.mean((pred_full - Y_test_imgnet)**2))
		
		class_err_imgnet_test.append(1-(np.argmax(pred,axis=0) == np.asarray(np.squeeze(labels_test_imgnet_reduced))).mean())
		class_err_imgnet_test_full.append(1-(np.argmax(pred_full,axis=0) == np.asarray(np.squeeze(labels_test_imgnet))).mean())
		
		##
		err_imgnet_plot.append(err_imgnet)
		
		savemat(file_name, {'F1': F1, 'F2': F2, 'F3': F3, 'F1_init': F1_init, 'FL_imgnet': FL_imgnet,\
			'step': step,'FL_imgnet_full':FL_imgnet_full,\
			'err_imgnet_test_full_plot':err_imgnet_test_full_plot, 'class_err_imgnet_test_full':class_err_imgnet_test_full,\
		...
		print 'err_imgnet_test_full:',err_imgnet_test_full_plot[-1], \
			'class_imgnet_test_full:',class_err_imgnet_test_full[-1]
