...
n_points = 15000#2775 #n_triplets #60000
filename = 'opt_' + filters_name + '_thirdorder_grad_npoints' + str(n_points) + '.mat'
...
for i in range(in_dims):
   for j in range(i+1, in_dims):
      for k in range(j+1, in_dims):
        numer = np.sum(x_no_mean[i]*w[m,n])
        denom = x_std[i]*x_std[m]*x_std[n]
        t = numer / denom
        loc = np.nonzero(np.abs(t) > np.abs(target))[0]
        if len(loc) > 0:
           #print 'f: ', len(loc)
           min_val = np.min(np.abs(target[loc]))
           ind = loc[np.nonzero(min_val == np.abs(target[loc]))[0][0]]
           target[ind] = t
           inds_i[ind] = i; inds_m[ind] = j; inds_n[ind] = k
