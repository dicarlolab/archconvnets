...
#define COSEDM_SZ (n_controllers*M*M*mem_length*sizeof(DATA_TYPE))
	
__global__ void cosine_sim_expand_dmem_kernel(float * keys, float * mem, 
	...
	for(int j_local = 0; j_local < M; j_local++){
		if(j_local != j)
			COSEDM(i,j,j_local,k) = 0;
	}

	return;
}

static PyObject *cosine_sim_expand_dmem(PyObject *self, PyObject *args){
	...
	// run kernel
	cosine_sim_expand_dmem_kernel <<< n_controllers, M*mem_length >>> 
