...
gw = np.asarray(np.random.random((13, 3)),dtype='single')
add_out = np.asarray(np.random.random((13,12)),dtype='single')

###########
t_start = time.time()
z3 = add_mem(gw, add_out)
...
#############
t_start = time.time()
nm.add_mem(GW, ADD_OUT, OUT_BUFFER)
