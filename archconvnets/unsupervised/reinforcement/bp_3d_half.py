...
file_name = '/home/darren/3d_half_92_64.mat'
...
FL_3d = np.single(np.random.normal(scale=FL_imgnet_scale, size=(2, n3, max_output_sz3, max_output_sz3)))
...
while True:
	...
	if s_imgnet == 0:
		##################
		# load train imgs into buffers (3d)
		z = np.load('/export/storage/panda_environ_cudaconvnet/data_batch_' + str(imgnet_batch))
		
		imgs_pad = np.zeros((IMGNET_BATCH_SZ, 3, IMG_SZ, IMG_SZ),dtype='single')
		Y_train = np.zeros((2, IMGNET_BATCH_SZ), dtype='uint8')

		x2 = z['data'].reshape((3, IMG_SZ, IMG_SZ, IMGNET_BATCH_SZ))

		labels = np.asarray(z['labels'])

		l = np.zeros((IMGNET_BATCH_SZ, 2),dtype='uint8')
		...
		##################
		# load train imgs into buffers (imgnet)
		z = np.load('/export/storage/imgnet92/data_batch_' + str(imgnet_batch))
		
		imgs_pad_imgnet = np.zeros((IMGNET_BATCH_SZ, 3, IMG_SZ, IMG_SZ),dtype='single')
		Y_train_imgnet = np.zeros((999, IMGNET_BATCH_SZ), dtype='uint8')
		...
		imgs_pad_imgnet = np.ascontiguousarray(x2.transpose((3,0,1,2)))
	...
	pred = np.einsum(FL_3d, range(4), max_output3, [4,1,2,3], [0,4])
	
	pred_m_Y = Y_train[:,s_imgnet*64:(s_imgnet+1)*64] - pred
	
	FL_pred = np.einsum(FL_3d, range(4), pred_m_Y, [0,4], [4,1,2,3])
	...
	############# forward (imagenet)
	set_buffer(imgs_pad_imgnet[s_imgnet*64:(s_imgnet+1)*64] - mean_img, IMGS_PAD_IMGNET, gpu=GPU_CUR)
	...
	max_output3_imgnet = return_buffer(MAX_OUTPUT3_IMGNET, gpu=GPU_CUR)
	
	pred_imgnet = np.einsum(FL_imgnet, range(4), max_output3_imgnet, [4,1,2,3], [0,4])
	
	pred_m_Y_imgnet = Y_train_imgnet[:,s_imgnet*64:(s_imgnet+1)*64] - pred_imgnet
	
	############################################
	
	### return
	dFL_3d += np.einsum(max_output3, range(4), pred_m_Y, [4,0], [4,1,2,3])
	dFL_imgnet += np.einsum(max_output3_imgnet, range(4), pred_m_Y_imgnet, [4,0], [4,1,2,3])
	...
	if step % SAVE_FREQ == 0:
		...
		##
		err_3d_plot.append(err_3d)
		
		savemat(file_name, {'F1': F1, 'F2': F2, 'F3': F3, 'F1_init': F1_init, 'FL_3d': FL_3d,\
			'step': step,'FL_imgnet':FL_imgnet,\
			'err_imgnet_test_plot':err_imgnet_test_plot, 'class_err_imgnet_test':class_err_imgnet_test,\
			'err_3d_plot':err_3d_plot})
		
		print file_name
		print 'step:', step, 'F1:', np.max(F1), 't:',time.time() - t_start
		print 'err_3d:', err_3d, 'err_imgnet_test:',err_imgnet_test_plot[-1], \
