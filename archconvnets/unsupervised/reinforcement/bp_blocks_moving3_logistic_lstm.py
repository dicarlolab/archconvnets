...
while True:
	...
	if step % SAVE_FREQ == 0:
		...
		print 'FLmem: %f %f (%f)   layer: %f %f (contrib: %f)' % (np.min(FL), np.max(FL), np.median(np.abs(ft*dFL.ravel())/np.abs(FL.ravel())),\
						np.min(mem_contrib), np.max(mem_contrib), np.median(np.abs(mem_contrib)/np.abs(curr_contrib)))
		print 'FLbypass: %f %f (%f)   layer: %f %f' % (np.min(FL_bypass), np.max(FL_bypass), np.median(np.abs(ft*dFL_bypass.ravel())/np.abs(FL_bypass.ravel())),\
						np.min(curr_contrib), np.max(curr_contrib))
