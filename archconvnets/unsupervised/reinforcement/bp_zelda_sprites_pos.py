from archconvnets.unsupervised.cudnn_module.cudnn_module import *
import time
import numpy as np
import numexpr as ne
from scipy.io import savemat, loadmat
import copy
from scipy.stats import zscore
import random
import gnumpy as gpu
import scipy
import PIL
import Image

SAVE_FREQ = 4000
RAND_PERIOD = 0#10000
MEM_SZ = 500000
EPS_GREED_FINAL = .1
EPS_GREED_FINAL_TIME = 5000000
GAMMA = 0.99
BATCH_SZ = 32
NETWORK_UPDATE = 10000
EPS = 5e-3
MOM_WEIGHT = 0.95
PLAYER_MOV_RATE = 8

IMG_SZ = 92
OBJ_SZ = 23
MAX_LOC = 92 - 10

F1_scale = 1e-2
F2_scale = 1e-2
F3_scale = 1e-2
FL_scale = 1e-2

N = 64#32
n1 = N # L1 filters
n2 = N# ...
n3 = N

s3 = 3 # L1 filter size (px)
s2 = 4 # ...
s1 = 5

N_C = 4 # directions L, R, U, D

file_name = '/home/darren/reinforcement64.mat'

max_output_sz3  = 12

GPU_CUR = 2
GPU_PREV = 3

# gpu buffer indices
MAX_OUTPUT1 = 0; DF2_DATA = 1; CONV_OUTPUT1 = 2; DPOOL1 = 3
F1_IND = 4; IMGS_PAD = 5; DF1 = 6; F2_IND = 11
D_UNPOOL2 = 12;F3_IND = 13; MAX_OUTPUT2 = 14; MAX_OUTPUT3 = 15
CONV_OUTPUT1 = 19; CONV_OUTPUT2 = 20; CONV_OUTPUT3 = 21
DF2 = 25; DPOOL2 = 26; DF3_DATA = 27; DPOOL3 = 28; DF3 = 29; FL_PRED = 30;
FL_IND = 31; PRED = 32; DFL = 33

#np.random.seed(6666)
F1 = np.single(np.random.normal(scale=F1_scale, size=(n1, 3, s1, s1)))
F2 = np.single(np.random.normal(scale=F2_scale, size=(n2, n1, s2, s2)))
F3 = np.single(np.random.normal(scale=F3_scale, size=(n3, n2, s3, s3)))
FL = np.single(np.random.normal(scale=FL_scale, size=(N_C, n1, max_output_sz3, max_output_sz3)))

FL_prev = copy.deepcopy(FL)
set_buffer(F1, F1_IND, filter_flag=1, gpu=GPU_PREV)
set_buffer(F2, F2_IND, filter_flag=1, gpu=GPU_PREV)
set_buffer(F3, F3_IND, filter_flag=1, gpu=GPU_PREV)
set_buffer(FL, FL_IND, filter_flag=1, gpu=GPU_PREV)

set_buffer(F1, F1_IND, filter_flag=1, gpu=GPU_CUR)
set_buffer(F2, F2_IND, filter_flag=1, gpu=GPU_CUR)
set_buffer(F3, F3_IND, filter_flag=1, gpu=GPU_CUR)
set_buffer(FL, FL_IND, filter_flag=1, gpu=GPU_CUR)

F1_init = copy.deepcopy(F1)

dF1 = np.zeros_like(F1)
dF2 = np.zeros_like(F2)
dF3 = np.zeros_like(F3)
dFL = np.zeros_like(FL)

dF1_mom = np.zeros_like(F1)
dF2_mom = np.zeros_like(F2)
dF3_mom = np.zeros_like(F3)
dFL_mom = np.zeros_like(FL)

r_total = 0
r_total_plot = []
network_updates = 0
step = 0
err = 0
err_plot = []

###########
# init scene
grass = np.ascontiguousarray(np.single(np.asarray(PIL.Image.open('grass.png')).transpose((2,0,1))[np.newaxis]))
zelda = np.ascontiguousarray(np.single(np.asarray(PIL.Image.open('zelda.png')).transpose((2,0,1))))
dog = np.ascontiguousarray(np.single(np.asarray(PIL.Image.open('dog.png')).transpose((2,0,1))))
heart = np.ascontiguousarray(np.single(np.asarray(PIL.Image.open('heart.png')).transpose((2,0,1))))

mean_img = grass#*(MAX_LOC**2-3.)/(MAX_LOC**2)

#for x in range(MAX_LOC):
#	for y in range(MAX_LOC):
#		mean_img[0,:,x:(x+OBJ_SZ), y:(y+OBJ_SZ)] += (dog[:,:IMG_SZ-x, :IMG_SZ-y] + heart[:,:IMG_SZ-x, :IMG_SZ-y] + zelda[:,:IMG_SZ-x, :IMG_SZ-y]) / (MAX_LOC**2)


reds_input = np.zeros((MEM_SZ, 2), dtype='int')
blues_input = np.zeros((MEM_SZ, 2), dtype='int')
player_input = np.zeros((MEM_SZ, 2), dtype='int')
action_input = np.zeros(MEM_SZ, dtype='int')

r_output = np.zeros(MEM_SZ)
y_outputs = np.zeros(MEM_SZ)
y_network_ver = -np.ones(MEM_SZ)
reds_output = np.zeros((MEM_SZ, 2), dtype='int')
blues_output = np.zeros((MEM_SZ, 2), dtype='int')
player_output = np.zeros((MEM_SZ, 2), dtype='int')

reds = np.random.randint(0,MAX_LOC, size=2)
blues = np.random.randint(0,MAX_LOC, size=2)
player = np.random.randint(0,MAX_LOC, size=2)

img = copy.deepcopy(grass)
img_buffer = np.zeros((SAVE_FREQ, 3, IMG_SZ, IMG_SZ), dtype='uint8')

t_start = time.time()
while True:
	mem_loc  = step % MEM_SZ
	
	# copy current state
	reds_input[mem_loc] = copy.deepcopy(reds)
	blues_input[mem_loc] = copy.deepcopy(blues)
	player_input[mem_loc] = copy.deepcopy(player)
	y_network_ver[mem_loc] = -1
	
	# show blocks
	img = copy.deepcopy(grass)
	img[0,:,player[0]:(player[0]+OBJ_SZ), player[1]:(player[1]+OBJ_SZ)] = zelda[:,:IMG_SZ-player[0],:IMG_SZ-player[1]]
	img[0,:,reds[0]:(reds[0]+OBJ_SZ), reds[1]:(reds[1]+OBJ_SZ)] = dog[:,:IMG_SZ-reds[0],:IMG_SZ-reds[1]]
	img[0,:,blues[0]:(blues[0]+OBJ_SZ), blues[1]:(blues[1]+OBJ_SZ)] = heart[:,:IMG_SZ-blues[0],:IMG_SZ-blues[1]]
	
	img_buffer[step % SAVE_FREQ] = copy.deepcopy(img[0])
	
	# choose action
	CHANCE_RAND = np.max((1 - ((1-EPS_GREED_FINAL)/EPS_GREED_FINAL_TIME)*(step - (MEM_SZ+RAND_PERIOD)), EPS_GREED_FINAL))
	if np.random.rand() <= CHANCE_RAND:
		action = np.random.randint(4)
	else:
		# forward pass
		set_buffer(img - mean_img, IMGS_PAD, gpu=GPU_CUR)
			
		conv_buffers(F1_IND, IMGS_PAD, CONV_OUTPUT1, gpu=GPU_CUR)
		max_pool_cudnn_buffers(CONV_OUTPUT1, MAX_OUTPUT1, gpu=GPU_CUR)
		conv_buffers(F2_IND, MAX_OUTPUT1, CONV_OUTPUT2, gpu=GPU_CUR)
		max_pool_cudnn_buffers(CONV_OUTPUT2, MAX_OUTPUT2, gpu=GPU_CUR)
		conv_buffers(F3_IND, MAX_OUTPUT2, CONV_OUTPUT3, gpu=GPU_CUR)
		max_pool_cudnn_buffers(CONV_OUTPUT3, MAX_OUTPUT3, gpu=GPU_CUR)
		
		max_output3 = return_buffer(MAX_OUTPUT3, gpu=GPU_CUR)
		
		pred = np.einsum(FL, range(4), max_output3, [4,1,2,3], [0])
		
		action = np.argmax(pred)

	# perform action
	if action == 0 and player[0] > PLAYER_MOV_RATE:
		player[0] -= PLAYER_MOV_RATE
	elif action == 1 and (player[0]) < (MAX_LOC-1):
		player[0] += PLAYER_MOV_RATE
	elif action == 2 and (player[1]) > PLAYER_MOV_RATE:
		player[1] -= PLAYER_MOV_RATE
	elif action == 3 and (player[1]) < (MAX_LOC-1):
		player[1] += PLAYER_MOV_RATE

	
	# determine reward, choose new block locations
	r = 0

	# red collision, place new red block
	collision = np.nonzero(((player[0] >= reds[0]) * (player[0] <= (reds[0]+OBJ_SZ)) * (player[1] >= reds[1]) * (player[1] <= (reds[1]+OBJ_SZ))) + \
				((player[0]+OBJ_SZ) >= reds[0]) * ((player[0]+OBJ_SZ) <= (reds[0]+OBJ_SZ)) * (player[1] >= reds[1]) * (player[1] <= (reds[1]+OBJ_SZ)) + \
				((player[0]+OBJ_SZ) >= reds[0]) * ((player[0]+OBJ_SZ) <= (reds[0]+OBJ_SZ)) * ((player[1]+OBJ_SZ) >= reds[1]) * ((player[1]+OBJ_SZ) <= (reds[1]+OBJ_SZ)) + \
				(player[0] >= reds[0]) * (player[0] <= (reds[0]+OBJ_SZ)) * ((player[1]+OBJ_SZ) >= reds[1]) * ((player[1]+OBJ_SZ) <= (reds[1]+OBJ_SZ)))[0]
	while len(collision) >= 1:
		r = -1
		reds = np.random.randint(0,MAX_LOC, size=2)
		collision = np.nonzero(((player[0] >= reds[0]) * (player[0] <= (reds[0]+OBJ_SZ)) * (player[1] >= reds[1]) * (player[1] <= (reds[1]+OBJ_SZ))) + \
				((player[0]+OBJ_SZ) >= reds[0]) * ((player[0]+OBJ_SZ) <= (reds[0]+OBJ_SZ)) * (player[1] >= reds[1]) * (player[1] <= (reds[1]+OBJ_SZ)) + \
				((player[0]+OBJ_SZ) >= reds[0]) * ((player[0]+OBJ_SZ) <= (reds[0]+OBJ_SZ)) * ((player[1]+OBJ_SZ) >= reds[1]) * ((player[1]+OBJ_SZ) <= (reds[1]+OBJ_SZ)) + \
				(player[0] >= reds[0]) * (player[0] <= (reds[0]+OBJ_SZ)) * ((player[1]+OBJ_SZ) >= reds[1]) * ((player[1]+OBJ_SZ) <= (reds[1]+OBJ_SZ)))[0]

	# blue collision, place new blue block
	collision = np.nonzero(((player[0] >= blues[0]) * (player[0] <= (blues[0]+OBJ_SZ)) * (player[1] >= blues[1]) * (player[1] <= (blues[1]+OBJ_SZ))) + \
				((player[0]+OBJ_SZ) >= blues[0]) * ((player[0]+OBJ_SZ) <= (blues[0]+OBJ_SZ)) * (player[1] >= blues[1]) * (player[1] <= (blues[1]+OBJ_SZ)) + \
				((player[0]+OBJ_SZ) >= blues[0]) * ((player[0]+OBJ_SZ) <= (blues[0]+OBJ_SZ)) * ((player[1]+OBJ_SZ) >= blues[1]) * ((player[1]+OBJ_SZ) <= (blues[1]+OBJ_SZ)) + \
				(player[0] >= blues[0]) * (player[0] <= (blues[0]+OBJ_SZ)) * ((player[1]+OBJ_SZ) >= blues[1]) * ((player[1]+OBJ_SZ) <= (blues[1]+OBJ_SZ)))[0]
	while len(collision) >= 1:
		r = 1
		blues = np.random.randint(0,MAX_LOC, size=2)
		collision = np.nonzero(((player[0] >= blues[0]) * (player[0] <= (blues[0]+OBJ_SZ)) * (player[1] >= blues[1]) * (player[1] <= (blues[1]+OBJ_SZ))) + \
				((player[0]+OBJ_SZ) >= blues[0]) * ((player[0]+OBJ_SZ) <= (blues[0]+OBJ_SZ)) * (player[1] >= blues[1]) * (player[1] <= (blues[1]+OBJ_SZ)) + \
				((player[0]+OBJ_SZ) >= blues[0]) * ((player[0]+OBJ_SZ) <= (blues[0]+OBJ_SZ)) * ((player[1]+OBJ_SZ) >= blues[1]) * ((player[1]+OBJ_SZ) <= (blues[1]+OBJ_SZ)) + \
				(player[0] >= blues[0]) * (player[0] <= (blues[0]+OBJ_SZ)) * ((player[1]+OBJ_SZ) >= blues[1]) * ((player[1]+OBJ_SZ) <= (blues[1]+OBJ_SZ)))[0]

	r_total += r
	
	# copy current state
	reds_output[mem_loc] = copy.deepcopy(reds)
	blues_output[mem_loc] = copy.deepcopy(blues)
	player_output[mem_loc] = copy.deepcopy(player)
	r_output[mem_loc] = r
	action_input[mem_loc] = action
	
	if step == MEM_SZ:
		print 'beginning gradient computations'
	if step == (MEM_SZ+RAND_PERIOD):
		print 'beginning non-random actions'
	
	######################################
	# update gradient?
	if step >= MEM_SZ:
		trans = np.random.randint(MEM_SZ)
		
		# show blocks
		img_prev = copy.deepcopy(grass)
		img_cur = copy.deepcopy(grass)
		
		img_prev[0,:,player_output[trans][0]:(player_output[trans][0]+OBJ_SZ), player_output[trans][1]:(player_output[trans][1]+OBJ_SZ)] = zelda[:,:IMG_SZ-player_output[trans][0],:IMG_SZ-player_output[trans][1]]
		img_cur[0,:,player_input[trans][0]:(player_input[trans][0]+OBJ_SZ), player_input[trans][1]:(player_input[trans][1]+OBJ_SZ)] = zelda[:,:IMG_SZ-player_input[trans][0],:IMG_SZ-player_input[trans][1]]
		
		img_prev[0,:,reds_output[trans][0]:(reds_output[trans][0]+OBJ_SZ), reds_output[trans][1]:(reds_output[trans][1]+OBJ_SZ)] = dog[:,:IMG_SZ-reds_output[trans][0],:IMG_SZ-reds_output[trans][1]]
		img_prev[0,:,blues_output[trans][0]:(blues_output[trans][0]+OBJ_SZ), blues_output[trans][1]:(blues_output[trans][1]+OBJ_SZ)] = heart[:,:IMG_SZ-blues_output[trans][0],:IMG_SZ-blues_output[trans][1]]
		
		img_cur[0,:,reds_input[trans][0]:(reds_input[trans][0]+OBJ_SZ), reds_input[trans][1]:(reds_input[trans][1]+OBJ_SZ)] = dog[:,:IMG_SZ-reds_input[trans][0],:IMG_SZ-reds_input[trans][1]]
		img_cur[0,:,blues_input[trans][0]:(blues_input[trans][0]+OBJ_SZ), blues_input[trans][1]:(blues_input[trans][1]+OBJ_SZ)] = heart[:,:IMG_SZ-blues_input[trans][0],:IMG_SZ-blues_input[trans][1]]
		
		set_buffer(img_cur - mean_img, IMGS_PAD, gpu=GPU_CUR)
		
		# forward pass current network
		conv_buffers(F1_IND, IMGS_PAD, CONV_OUTPUT1, gpu=GPU_CUR)
		max_pool_cudnn_buffers(CONV_OUTPUT1, MAX_OUTPUT1, gpu=GPU_CUR)
		conv_buffers(F2_IND, MAX_OUTPUT1, CONV_OUTPUT2, gpu=GPU_CUR)
		max_pool_cudnn_buffers(CONV_OUTPUT2, MAX_OUTPUT2, gpu=GPU_CUR)
		conv_buffers(F3_IND, MAX_OUTPUT2, CONV_OUTPUT3, gpu=GPU_CUR)
		max_pool_cudnn_buffers(CONV_OUTPUT3, MAX_OUTPUT3, gpu=GPU_CUR)
		
		# forward pass prev network
		if y_network_ver[trans] != (network_updates % NETWORK_UPDATE):
			set_buffer(img_prev - mean_img, IMGS_PAD, gpu=GPU_PREV)
		
			conv_buffers(F1_IND, IMGS_PAD, CONV_OUTPUT1, gpu=GPU_PREV)
			max_pool_cudnn_buffers(CONV_OUTPUT1, MAX_OUTPUT1, gpu=GPU_PREV)
			conv_buffers(F2_IND, MAX_OUTPUT1, CONV_OUTPUT2, gpu=GPU_PREV)
			max_pool_cudnn_buffers(CONV_OUTPUT2, MAX_OUTPUT2, gpu=GPU_PREV)
			conv_buffers(F3_IND, MAX_OUTPUT2, CONV_OUTPUT3, gpu=GPU_PREV)
			max_pool_cudnn_buffers(CONV_OUTPUT3, MAX_OUTPUT3, gpu=GPU_PREV)
		
			# compute target
			max_output3 = return_buffer(MAX_OUTPUT3, gpu=GPU_PREV)
			pred_prev = np.einsum(FL_prev, range(4), max_output3, [4,1,2,3], [0])
			y_outputs[trans] = r_output[trans] + GAMMA * np.max(pred_prev)
			y_network_ver[trans] = network_updates % NETWORK_UPDATE
		
		max_output3 = return_buffer(MAX_OUTPUT3, gpu=GPU_CUR)
		
		pred = np.einsum(FL, range(4), max_output3, [4,1,2,3], [0])
		pred_m_Y = y_outputs[trans] - pred[action_input[trans]]
		
		err += pred_m_Y**2
		
		FL_pred = np.ascontiguousarray((FL[action_input[trans]] * pred_m_Y)[np.newaxis])
		
		set_buffer(FL_pred, FL_PRED, gpu=GPU_CUR)
		
		########### backprop
		max_pool_back_cudnn_buffers(MAX_OUTPUT3, FL_PRED, CONV_OUTPUT3, DPOOL3, gpu=GPU_CUR)
		conv_dfilter_buffers(F3_IND, MAX_OUTPUT2, DPOOL3, DF3, stream=3, gpu=GPU_CUR)
		conv_ddata_buffers(F3_IND, MAX_OUTPUT2, DPOOL3, DF3_DATA, gpu=GPU_CUR)
		max_pool_back_cudnn_buffers(MAX_OUTPUT2, DF3_DATA, CONV_OUTPUT2, DPOOL2, gpu=GPU_CUR)
		conv_ddata_buffers(F2_IND, MAX_OUTPUT1, DPOOL2, DF2_DATA, gpu=GPU_CUR)
		conv_dfilter_buffers(F2_IND, MAX_OUTPUT1, DPOOL2, DF2, stream=2, gpu=GPU_CUR)
		max_pool_back_cudnn_buffers(MAX_OUTPUT1, DF2_DATA, CONV_OUTPUT1, DPOOL1, gpu=GPU_CUR)
		conv_dfilter_buffers(F1_IND, IMGS_PAD, DPOOL1, DF1, stream=1, gpu=GPU_CUR)

		### return
		dFL[action_input[trans]] += max_output3[0]*pred_m_Y
		dF3 += return_buffer(DF3, stream=3, gpu=GPU_CUR)
		dF2 += return_buffer(DF2, stream=2, gpu=GPU_CUR)
		dF1 += return_buffer(DF1, stream=1, gpu=GPU_CUR)
		
		#### update filter weights
		if(step - MEM_SZ) % BATCH_SZ == 0:
			F1 += (dF1 + MOM_WEIGHT*dF1_mom)*EPS / BATCH_SZ
			F2 += (dF2 + MOM_WEIGHT*dF2_mom)*EPS / BATCH_SZ
			F3 += (dF3 + MOM_WEIGHT*dF3_mom)*EPS / BATCH_SZ
			FL += (dFL + MOM_WEIGHT*dFL_mom)*EPS / BATCH_SZ
			
			set_buffer(F1, F1_IND, filter_flag=1, gpu=GPU_CUR)
			set_buffer(F2, F2_IND, filter_flag=1, gpu=GPU_CUR)
			set_buffer(F3, F3_IND, filter_flag=1, gpu=GPU_CUR)
			set_buffer(FL, FL_IND, filter_flag=1, gpu=GPU_CUR)
			
			dF1_mom = copy.deepcopy(dF1)
			dF2_mom = copy.deepcopy(dF2)
			dF3_mom = copy.deepcopy(dF3)
			dFL_mom = copy.deepcopy(dFL)
			
			dF1 = np.zeros_like(dF1)
			dF2 = np.zeros_like(dF2)
			dF3 = np.zeros_like(dF3)
			dFL = np.zeros_like(dFL)
			
			network_updates += 1
			
			if network_updates % NETWORK_UPDATE == 0:
				print 'updating network'
				FL_prev = copy.deepcopy(FL)
				set_buffer(F1, F1_IND, filter_flag=1, gpu=GPU_PREV)
				set_buffer(F2, F2_IND, filter_flag=1, gpu=GPU_PREV)
				set_buffer(F3, F3_IND, filter_flag=1, gpu=GPU_PREV)
				set_buffer(FL, FL_IND, filter_flag=1, gpu=GPU_PREV)
				
	step += 1
	
	if step % SAVE_FREQ == 0:
		r_total_plot.append(r_total)
		err_plot.append(err)
		
		savemat(file_name, {'F1': F1, 'r_total_plot': r_total_plot, 'F2': F2, 'F3': F3, 'FL':FL, 'F1_init': F1_init, 'step': step, 'img': img, 'err_plot': err_plot, 'img_buffer': img_buffer})
		print 'step:', step, 'err:',err, 'r:',r_total, 'updates:',network_updates, 'eps:', CHANCE_RAND, 'F1:', np.max(F1), 't:',time.time() - t_start, file_name
		
		err = 0
		r_total = 0
		
		t_start = time.time()
