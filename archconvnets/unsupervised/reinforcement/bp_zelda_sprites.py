...
mean_img = grass*((16.-3)/16)

for x in range(MAX_LOC):
	for y in range(MAX_LOC):
		mean_img[0,:,SCALE*x:SCALE*(x+1), SCALE*y:SCALE*(y+1)] += (dog + heart + zelda) / (MAX_LOC**2)
...
img = copy.deepcopy(grass) #np.zeros((1,3,IMG_SZ,IMG_SZ),dtype='single')
...
while True:
	...
	if np.random.rand() <= CHANCE_RAND:
		...
		for b in range(N_REDS):
			img[0,:,SCALE*reds[0,b]:SCALE*(reds[0,b]+1), SCALE*reds[1,b]:SCALE*(reds[1,b]+1)] = dog
			img[0,:,SCALE*blues[0,b]:SCALE*(blues[0,b]+1), SCALE*blues[1,b]:SCALE*(blues[1,b]+1)] = heart
		img[0,:,SCALE*player[0]:SCALE*(player[0]+1), SCALE*player[1]:SCALE*(player[1]+1)] = zelda
	...
	if step >= MEM_SZ:
		...
		# show blocks
		img_prev = copy.deepcopy(grass) #np.zeros((1,3,IMG_SZ,IMG_SZ),dtype='single')
		img_cur = copy.deepcopy(grass) #np.zeros((1,3,IMG_SZ,IMG_SZ),dtype='single')
		
		img_prev[0,:,SCALE*player_output[trans][0]:SCALE*(player_output[trans][0]+1), SCALE*player_output[trans][1]:SCALE*(player_output[trans][1]+1)] = zelda
		img_cur[0,:,SCALE*player_input[trans][0]:SCALE*(player_input[trans][0]+1), SCALE*player_input[trans][1]:SCALE*(player_input[trans][1]+1)] = zelda
		
		for b in range(N_REDS):
			img_prev[0,:,SCALE*reds_output[trans][0,b]:SCALE*(reds_output[trans][0,b]+1), SCALE*reds_output[trans][1,b]:SCALE*(reds_output[trans][1,b]+1)] = dog
			img_prev[0,:,SCALE*blues_output[trans][0,b]:SCALE*(blues_output[trans][0,b]+1), SCALE*blues_output[trans][1,b]:SCALE*(blues_output[trans][1,b]+1)] = heart
			
			img_cur[0,:,SCALE*reds_input[trans][0,b]:SCALE*(reds_input[trans][0,b]+1), SCALE*reds_input[trans][1,b]:SCALE*(reds_input[trans][1,b]+1)] = dog
			img_cur[0,:,SCALE*blues_input[trans][0,b]:SCALE*(blues_input[trans][0,b]+1), SCALE*blues_input[trans][1,b]:SCALE*(blues_input[trans][1,b]+1)] = heart
