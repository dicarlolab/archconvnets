...
if static == True:
	file_name = '/home/darren/reinforcement_timer_lstm_ncec.mat'
else:
...
while True:
	...
	if step % SAVE_FREQ == 0:
		...
		print 'CEC3: %f %f, kept: %f %f, new: %f %f' % (np.min(CEC3), np.max(CEC3), np.min(CEC_kept3), np.max(CEC_kept3), \
						np.min(CEC_new3), np.max(CEC_new3))
		print
