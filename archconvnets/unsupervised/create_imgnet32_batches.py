...
imgs_new = np.zeros((BATCH_SZ,3*32*32), dtype='uint8')
...
for batch in range(1,N_BATCHES+1):
	...
	z = np.load('/storage/batch128_img138_full/data_batch_' + str(batch))
	...
	for img_i in range(128):
		...
		if img_local == (BATCH_SZ-1):
			...
			t_start = time.time()
			savemat('/home/darren/imgnet32/data_batch_' + str(write_batch),{'data': imgs_new, 'labels': labels})
			write_batch += 1
