...
t_start = time.time()
pred_deriv_test = pred_deriv_gpu(F1,F2,F3,FL,output_switches3_x - PAD, output_switches3_y - PAD, output_switches2_x - PAD, output_switches2_y - PAD, output_switches1_x, output_switches1_y, s1, s2, s3, imgs_pad, N_C,1)
print time.time() - t_start

print np.isclose(pred_deriv, pred_deriv_test).sum()/np.single(np.prod(pred_deriv_test.shape))
