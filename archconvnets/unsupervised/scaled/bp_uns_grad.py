...
for step in range(np.int(1e7)):
	...
	a1_y_ = 1
	channel_ = 1
	# output_switches3_x.shape = (32,29,29,1)
	# imgs_1 = (3,5,5,32,29,29,1)
	pool1_patches = np.random.random((32,29,29,N_IMGS, 3*5*5))
	
	grad = np.zeros_like(F1)
	for a1_x_ in range(s1):
		print a1_x_
		for a1_y_ in range(s1):
			for channel in range(channel_):
				F1_ident = np.zeros((3,5,5))
				F1_ident[channel_, a1_x_, a1_y_] = 1
				F1_ident = F1_ident.ravel()
				
				pool1_deriv = np.dot(pool1_patches, F1_ident)
