from archconvnets.unsupervised.cudnn_module.cudnn_module import conv
import time
import numpy as np
	...
	for batch in range(1,6):
		for step in range(np.int((10000)/N_IMGS)):
			...
			for f2_ in range(n2):
				...
				conv_output3_derivt = conv_output3_deriv.reshape((n2, output_sz3, output_sz3, N_IMGS, n1, s2, s2))
				for a2_x_ in range(s2):
					for a2_y_ in range(s2):
						for f1_ in range(n1):
							a2_x_ + a2_y_ + f1_
							max_output3 = max_pool_locs_alt(conv_output3_derivt[:,:,:,:,f1_,a2_x_,a2_y_], output_switches3_x, output_switches3_y)
