...
for iter in range(np.int(1e7)):
	...
	for batch in range(1,6):
		for step in range(np.int((10000)/N_IMGS)):
			...
			max_output1t = max_pool_locs_alt(conv_output1[:,np.newaxis], output_switches1_x_init, output_switches1_y_init)
			max_output1 = np.zeros((N_TEST_IMGS, n1, max_output_sz1, max_output_sz1),dtype='single')
			...
			max_output2t = max_pool_locs_alt(conv_output2[:,np.newaxis], output_switches2_x_init, output_switches2_y_init)
			max_output2 = np.zeros((N_TEST_IMGS, n2, max_output_sz2, max_output_sz2),dtype='single')
			...
			max_output3 = max_pool_locs_alt(conv_output3[:,np.newaxis], output_switches3_x_init, output_switches3_y_init)
			...
			for f1_ in range(n1):
				set_filter_buffer(FBUFF_F2_GRAD_L1, F2[:,f1_][:,np.newaxis])
				...
				conv_output2_deriv = conv_output2_deriv.reshape((N_IMGS, 3*s1*s1, n1, output_sz2, output_sz2))
				...
				conv_output3_deriv = conv_output3_deriv.reshape((N_IMGS, 3*s1*s1, n2, output_sz3, output_sz3))
			...
			for f2_ in range(n2):
				set_filter_buffer(FBUFF_F3_GRAD_L2, F3[:,f2_][:,np.newaxis])
				...
				conv_output3_deriv = conv_output3_deriv.reshape((N_IMGS, n1*s2*s2, n2, output_sz3, output_sz3))
