...
filename = '/home/darren/cifar_bp8.mat'
...
print 'loading sigma'
sigma31 = np.load('/home/darren/sigma31_8_crop_1p2p3p4p5.npy')
...
max_output2t, output_switches2_x_init, output_switches2_y_init = max_pool_locs(conv_output2, PAD=PAD)
...
max_output3, output_switches3_x_init, output_switches3_y_init = max_pool_locs(conv_output3, PAD=PAD)
...
for step in range(10000000):
	...
	einsum_deriv_gpu(2,1,2,3) # deriv, l2
	einsum_deriv_gpu(3,1,3,2) # deriv, l3
	einsum_deriv_gpu(4,1,4,2) # deriv, fl
	...
	############################################# F2 deriv
	derivc = einsum_return(2,3)
	
	grad_L2 = 2*(derivc*predc).sum(0).sum(0)
	
	############################################# F3 deriv
	derivc = einsum_return(3,2)
	
	grad_L3 = 2*(derivc*predc).sum(0).sum(0)
	
	############################################# FL deriv
	derivc = einsum_return(4,2)
