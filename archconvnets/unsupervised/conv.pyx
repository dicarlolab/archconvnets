...
'''
def conv_block(npd.ndarray[npd.float64_t, ndim=4] filters, npd.ndarray[npd.float64_t, ndim=4] imgs, int stride): 
	...
	cdef npd.ndarray[npd.float64_t, ndim=3] patch
	cdef npd.ndarray[npd.float64_t, ndim=4] output = np.zeros((n_filters, output_sz, output_sz, n_imgs))
	cdef npd.ndarray[npd.float64_t, ndim=3] filter_temp = filters.reshape((in_channels*filter_sz**2,n_filters)).T.reshape((n_filters,in_channels*filter_sz**2,1))	
	
	for x in range(output_sz):
		y_loc = 0
		for y in range(output_sz):
			patch = imgs[:,x_loc:x_loc+filter_sz, y_loc:y_loc+filter_sz].reshape((1,in_channels*filter_sz**2,n_imgs))
			output[:, x, y, :] = np.sum(patch*filter_temp,axis=1)
			y_loc += stride
		x_loc += stride
	return output
