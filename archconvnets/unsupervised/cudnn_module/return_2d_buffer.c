static PyObject *return_2d_buffer(PyObject *self, PyObject *args){
    cudaError_t err;
	...
	dims[0] = data_2d_dims[0][gpu_ind][buffer_ind];
	dims[1] = data_2d_dims[1][gpu_ind][buffer_ind];
	
	data_in = (PyArrayObject *) PyArray_FromDims(2, dims, NPY_FLOAT);
	data = (float *) data_in -> data;
	
	err = (cudaError_t)cudaMemcpy(data, data_2d_buffers[gpu_ind][buffer_ind], dims[0]*dims[1] * DATA_TYPE_SZ, cudaMemcpyDeviceToHost);  MALLOC_ERR_CHECK
