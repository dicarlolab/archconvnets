static PyObject *activation_buffers(PyObject *self, PyObject *args)  {
	cudaError_t err;
	cudnnStatus_t status;
	int gpu_ind, imgs_ind, out_ind;
	int activation_ind;
	
	if (!PyArg_ParseTuple(args, "iiii", &imgs_ind, &out_ind, &activation_ind, &gpu_ind)) 
		return NULL;
	...
	// Activation
	//--------------------------------------
	status = cudnnActivationForward(handle, activation_mode, desc_buffers[gpu_ind][imgs_ind], data_buffers[gpu_ind][imgs_ind], desc_buffers[gpu_ind][out_ind], data_buffers[gpu_ind][out_ind]);  ERR_CHECK
