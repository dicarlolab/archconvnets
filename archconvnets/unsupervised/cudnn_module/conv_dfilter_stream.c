static PyObject *conv_dfilter_stream(PyObject *self, PyObject *args)  {
	...
	if (!PyArg_ParseTuple(args, "O!O!O!i", &PyArray_Type, &filters_in, &PyArray_Type, &imgs_in, &PyArray_Type, &conv_out_in, &gpu)) 
		return NULL;
	
	if (NULL == filters_in || NULL == imgs_in || NULL == conv_out_in)  return NULL;
	
	cudaSetDevice(gpu); CHECK_CUDA_ERR
	cudnnSetStream(handle, streams[gpu]);
