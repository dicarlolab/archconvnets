static PyObject *activation_back_buffers(PyObject *self, PyObject *args)  {
	cudaError_t err;
	cudnnStatus_t status;
	int gpu_ind, src_ind, src_diff_ind, dest_ind, out_ind, activation_ind;
	
	if (!PyArg_ParseTuple(args, "iiiiii", &src_ind, &src_diff_ind, &dest_ind, &out_ind, &activation_ind, &gpu_ind)) 
		return NULL;
	...
	}else if(activation_ind == 1){
		activation_mode = CUDNN_ACTIVATION_RELU;
	}else if(activation_ind == 2){
		activation_mode = CUDNN_ACTIVATION_TANH;
	}else{
		printf("invalid activation mode\n");
		return NULL;
	}
	...
	// Activation
	//--------------------------------------
	status = cudnnActivationBackward(handle, activation_mode, desc_buffers[gpu_ind][src_ind], data_buffers[gpu_ind][src_ind],
