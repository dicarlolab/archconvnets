...
for batch in range(1,7):
	...
	for s_batch in range(10):
		...
		x = x[:,:,:,s_batch*batch_sz:(s_batch+1)*batch_sz]

		labels = np.asarray(z['labels'])[s_batch*batch_sz:(s_batch+1)*batch_sz].astype(int)

		imgs_pad = np.zeros((3, IMG_SZ, IMG_SZ, batch_sz),dtype='single')
		...
		max_output1 = np.zeros((batch_sz, n1, max_output_sz1, max_output_sz1),dtype='single')
		...
		max_output2 = np.zeros((batch_sz, n2, max_output_sz2, max_output_sz2),dtype='single')
