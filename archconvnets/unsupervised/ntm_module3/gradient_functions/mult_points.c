__global__ void mult_points_kernel(float * out, float * a, float * b, int data_out_numel){
	...
	for(int dup = 0; dup < n_duplicates; dup++){
		...
		out[ind_g] = a[ind_g] * b[ind_g];
	}
}

static PyObject * mult_points(PyObject *self, PyObject *args){
	cudaError_t err;
