...
def interpolate_dinterp_gate_out(args, LAYER_OUT, DERIV_ABOVE, OUT_BUFFER=None, additional_args=[None],gpu_ind=GPU_IND):
	...
	_ntm_module3.interpolate_dinterp_gate_out(O_CONTENT[0], O_CONTENT[1], O_PREV[0], DERIV_ABOVE[0], OUT_BUFFER[0], gpu_ind)
	
	n_dim_not_summed = len(DERIV_ABOVE[1]) - len(LAYER_OUT[1])
	OUT_BUFFER[1] = DERIV_ABOVE[1][:1+n_dim_not_summed] + INTERP_GATE_OUT[1][1:]
...
def interpolate_do_content(args, LAYER_OUT, DERIV_ABOVE, OUT_BUFFER=None, additional_args=[None], gpu_ind=GPU_IND):
	...
	_ntm_module3.interpolate_do_content(INTERP_GATE_OUT[0], O_PREV[1], DERIV_ABOVE[0], OUT_BUFFER[0], gpu_ind)
	
	n_dim_not_summed = len(DERIV_ABOVE[1]) - len(LAYER_OUT[1])
	OUT_BUFFER[1] = DERIV_ABOVE[1][:1+n_dim_not_summed] + O_CONTENT[1][1:]
...
def interpolate_do_prev(args, LAYER_OUT, DERIV_ABOVE, OUT_BUFFER=None, additional_args=[None], gpu_ind=GPU_IND):
	...
	_ntm_module3.interpolate_do_prev(INTERP_GATE_OUT[0], O_PREV[1], DERIV_ABOVE[0], OUT_BUFFER[0], gpu_ind)
	
	n_dim_not_summed = len(DERIV_ABOVE[1]) - len(LAYER_OUT[1])
	OUT_BUFFER[1] = DERIV_ABOVE[1][:1+n_dim_not_summed] + O_PREV[1][1:]
...
def add_interpolate_layer(LAYERS, name, source, init=0):
	...
	if init == 0:
		...
		assert in_shape[0][0] == in_shape[1][0] == in_shape[2][0]
		
		assert len(out_shape) == len(LAYERS[source[1]]['out_shape']) == 3
		assert out_shape == LAYERS[source[1]]['out_shape']
		assert out_shape[1] == LAYERS[source[0]]['out_shape'][1]
		assert LAYERS[source[0]]['out_shape'][2] == 1
