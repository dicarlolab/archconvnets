...
static PyObject *softmax_dlayer_in(PyObject *self, PyObject *args){
	...
	long n_imgs = PyLong_AsLong(PyTuple_GetItem(layer_out_shape,0));
	long dim0 = PyLong_AsLong(PyTuple_GetItem(layer_out_shape,1));
	long dim1 = PyLong_AsLong(PyTuple_GetItem(layer_out_shape,2));
	int dim_above = buffer_sz[gpu_ind][deriv_above_ind] / buffer_sz[gpu_ind][layer_out_ind];
	
	if(n_imgs*dim0*dim1*sizeof(DATA_TYPE) != LAYER_OUT_SZ){
		printf("specified input sizes do not equal stored gpu buffer. %s\n", __FILE__);
		return NULL;
	}
	...
	for(int img = 0; img < n_imgs; img++){
		softmax_dlayer_in_kernel <<< dim_above, dim0*dim1 >>> (gpu_buffers[gpu_ind][layer_out_ind] + img*dim0*dim1, 
			gpu_buffers[gpu_ind][deriv_above_ind] + img*dim_above*dim0*dim1,
			GPU_BUFFER_OUT + img*dim_above*dim0*dim1, dim0, dim1);
	}
