...
__global__ void sq_points_kernel(float * layer_in, float * out_buffer, int data_out_numel){
	...
	for(int dup = 0; dup < n_duplicates; dup++){
		...
		out_buffer[ind_g] = layer_in[ind_g] * layer_in[ind_g];
	}
}

static PyObject *sq_points(PyObject *self, PyObject *args){
	cudaError_t err;
	...
	// run kernel
	sq_points_kernel <<< n_blocks, MAX_THREADS_PER_BLOCK >>> (gpu_buffers[gpu_ind][layer_in_ind], gpu_buffers[gpu_ind][out_buffer_ind], RELU_NUMEL);
