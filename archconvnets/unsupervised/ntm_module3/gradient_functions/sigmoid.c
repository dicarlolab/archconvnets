#define SIGMOID_NUMEL (buffer_sz[gpu_ind][layer_in_ind] / sizeof(DATA_TYPE))

__global__ void sigmoid_kernel(float * layer_in, float * out_buffer, int data_out_numel){
	...
	for(int dup = 0; dup < n_duplicates; dup++){
		...
		out_buffer[ind_g] = 1/(1+__expf(-layer_in[ind_g]));
	}
}

static PyObject *sigmoid(PyObject *self, PyObject *args){
	cudaError_t err;
	...
	// run kernel
	sigmoid_kernel <<< n_blocks, MAX_THREADS_PER_BLOCK >>> (gpu_buffers[gpu_ind][layer_in_ind], gpu_buffers[gpu_ind][out_buffer_ind], SIGMOID_NUMEL);
