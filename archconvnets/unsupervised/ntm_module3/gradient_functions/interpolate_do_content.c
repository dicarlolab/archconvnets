__global__ void interpolate_do_content_kernel(float * interp_gate_out, float * deriv_above, float * out_data, int dim0, int dim1,
	...
	out_data[ind] = deriv_above[ind] * interp_gate_out[img*dim0 + i];
}
...
static PyObject * interpolate_do_content(PyObject *self, PyObject *args){
	...
	interpolate_do_content_kernel <<< n_imgs*dim_above, dim0*dim1 >>> (gpu_buffers[gpu_ind][interp_gate_out_ind], gpu_buffers[gpu_ind][deriv_above_ind], 
