...
def conv_block(filters, base_batch):
	...
	pickle('/home/darren/tmp_l1_2.model', model)
	f = open('/home/darren/j','w')
	f2 = open('/home/darren/j2','w')
	subprocess.call(['rm', '-r', '/tmp/features_2'])
	subprocess.call(['python', '/home/darren/archconvnets_write/archconvnets_write/convnet/shownet.py', '-f', '/home/darren/tmp_l1_2.model', '--test-range=' + str(np.min(base_batch)) + '-' + str(np.max(base_batch)), '--train-range=0', '--write-features=' + layer_name, '--feature-path=/tmp/features_2', '--gpu=0'], stdout=f, stderr=f2)
	if len(base_batch) == 1:
		try:
			x = np.load('/tmp/features_2/data_batch_' + str(base_batch[0]))
...
def test_grad_slowness(x):
	...
	for batch in base_batches:
		print batch
		c = np.load('/tmp/features_2/data_batch_' + str(batch))
...
base_batches = np.arange(80000, 80000+45)#[80000,80001,80002,80003]
...
for step in range(500):
	...
	loss, grad = test_grad_transpose(x0)
	print loss/2016
	#x0 -= np.max(np.abs(x0))*step_sz*grad/np.max(np.abs(grad))
	#x0 /= np.max(np.abs(x0))
