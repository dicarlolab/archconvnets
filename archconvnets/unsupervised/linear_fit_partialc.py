...
inds = range(FL321.shape[1])
random.shuffle(inds)

inds = inds[:np.int(FRAC_KEEP * len(inds))]
...
for step in range(100000):
	...
	predc = (np.einsum(sigma31[:,inds], sigma_inds, FL321[:,inds], F_inds, [1,0]) - Y).reshape((N_C, N_C, 1)) # (c, img)
	grad = 2*(sigma31[:,inds][np.newaxis]*predc).mean(1)
	FL321[:,inds] -= EPS * grad
