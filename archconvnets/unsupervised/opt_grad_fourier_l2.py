...
def test_grad(x):
	...
	grad_f += grad_f2
        ...
        #########
	scale_channel = 1###############1*1e-1#1e0#1e3
	scale_fourier = 5e-3#5e-3############## 1*1e-4#1e-3#1
	...
	if fourier_loss < thresh:
	    ...
	    pickle('/home/darren/tmp_l2.model',model4)
...
model_path = '/home/darren/imgnet_3layer_128_relu_L1sub_prelim.model'
model4 = unpickle(model_path)

#########################
'''os.system('python /home/darren/archconvnets_write/archconvnets_write/convnet/shownet.py -f ' + model_path + ' --write-features=pool1_3a --feature-path=/tmp/pool1 --train-range=0 --test-range=1234-1244')
###########

x_c = np.zeros((n_channels_in*5*5,0),dtype='float32')
for i in range(1234,1245):
    print i
    x = np.load('/tmp/pool1/data_batch_' + str(i))['data'].reshape((128,n_channels_in,30,30))
    x = x.transpose((1,2,3,0))
    x = x[:,13:13+5][:,:,13:13+5]
    x = x.reshape((n_channels_in*5*5,128))
