...
while True:
	...
	# load targets
	set_buffer(cat_target, WEIGHTS[SYN_CAT_DIFF_IND][1]) # cat target
	set_buffer(obj_target, WEIGHTS[SYN_OBJ_DIFF_IND][1]) # obj target
	...
	if frame % SAVE_FREQ == 0 and frame != 0:
		...
		save_conv_state(LAYERS, WEIGHTS, WEIGHTS_F1_INIT, save_name, output_buffer, target_buffer, EPS, err_log, corr_log, cifar_err_log, cifar_class_log, obj_err_log, obj_class_log, cat_err_log, cat_class_log, EPOCH_LEN, N_MOVIES)
