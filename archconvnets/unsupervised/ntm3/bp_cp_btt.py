...
OUTPUT = [None]*TIME_LENGTH*2; WEIGHT_DERIVS = None; WEIGHT_DERIVS_RMS = None
...
while True:
	...
	OUTPUT[elapsed_time+1] = forward_network(LAYERS, WEIGHTS, OUTPUT[elapsed_time+1], OUTPUT[elapsed_time])
	
	if training != 1:
		err += return_buffer(OUTPUT[elapsed_time+1][-1])[0]
	...
	output_buffer[frame % SAVE_FREQ] =  return_buffer(OUTPUT[elapsed_time+1][OUT_IND])
	
	###### reverse
	WEIGHT_DERIVS = reverse_network_btt(len(LAYERS)-1, LAYERS, WEIGHTS, OUTPUT, WEIGHT_DERIVS, elapsed_time+1)
