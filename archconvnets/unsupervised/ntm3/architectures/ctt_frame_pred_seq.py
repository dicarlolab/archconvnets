...
def init_model():
	...
	for init in [0,1]:
		...
		###################################
		## frame prediction
		add_sigmoid_F_bias_layer(LAYERS, 'M3_0', 250, source='F3_MAX', batch_imgs=B, init=init)
		
		# BUDGET = IM_SZ*IM_SZ*3 * U_F3*5*5
		# BUDGET / (48*32*32) = 75
		add_sigmoid_F_bias_layer(LAYERS, 'C1_0', 64, source='F1', batch_imgs=B, init=init)
		add_sigmoid_F_bias_layer(LAYERS, 'C1_1', 250, batch_imgs=B, init=init)
		
		add_add_layer(LAYERS, 'STACK_SUM0', ['C1_1', 'M3_0'], init=init)
		
		add_sigmoid_F_bias_layer(LAYERS, 'STACK_SUM1', 250, batch_imgs=B, init=init)
