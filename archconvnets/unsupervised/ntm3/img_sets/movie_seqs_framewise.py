...
def load_movie_seqs_same_sz_target(batch, frame, CAT_DIFF_IND, OBJ_DIFF_IND, DIFF_IND, PX_INDS, WEIGHTS, testing=False):
	global movie_train_objs, Y_train_movie_cat, Y_train_movie_obj, movie_train_inputs, movie_train_targets, movie_train_base_frame
	...
	if frame >= 2: # test phase
		px_input = np.ascontiguousarray(movie_inputs[:,1])
		target = np.ascontiguousarray(movie_inputs[:,2])
	else: # input phase
		px_input = np.ascontiguousarray(movie_inputs[:,frame])
		target = np.ascontiguousarray(movie_inputs[:,frame])
		
	set_buffer(np.ascontiguousarray(target.reshape((BATCH_SZ, np.prod(target.shape[1:]), 1))), WEIGHTS[DIFF_IND][1])
	for ind in PX_INDS:
		set_buffer(px_input, WEIGHTS[ind][1])
	
	return objs,cats, cat_target, obj_target, movie_inputs, movie_inputs
