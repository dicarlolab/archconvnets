#
# visualize imagenet data from train_net.py
#
...
class ShowConvNet(ConvNet):
    def __init__(self, op, load_dic, train_provider, test_provider):
        ConvNet.__init__(self, op, load_dic, train_provider, test_provider)
    ...
    def plot_cost(self):
        ...
        if test_errors:
            ...
            test_errors = numpy.tile(test_errors, (1, self.testing_freq * numbatches))
...
#        pl.ylabel(self.show_cost)
        ...
        if self.cost_idx == 0:
            pl.title( 'log-likelihood cost' )
        elif self.cost_idx == 1:
            pl.title( 'error' )
        else:
            pl.title( self.cost + ": unknown cost" )
...
if __name__ == "__main__":
    try:
        ...
        model = ShowConvNet(op, load_dic, train_provider, test_provider )
        model.start()
